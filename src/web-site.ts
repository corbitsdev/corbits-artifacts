...
export function normalizeWebSitePath(path: string): string {
  ...
  for (const segment of normalized.split("/")) {
    ...
    // "." segments survive as aliases ("./index.html" vs "index.html") that
    // the duplicate check misses and static servers collapse at serve time.
    if (segment === ".") {
      throw new WebSiteContentError(`invalid path ("." segment): ${path}`);
    }
    if (segment.length === 0) {
...
const ParsedWebSiteContent = type("string.json.parse")
  .to(WebSiteContentSchema)
  .pipe(normalizeWebSiteContent);

export function parseWebSiteContentJson(raw: string): Required<WebSiteContent> {
  const result = ParsedWebSiteContent(raw);
