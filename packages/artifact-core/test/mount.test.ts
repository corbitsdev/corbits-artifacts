...
import { mountArtifacts } from "../src/mount.js";
import { InlineContentStore } from "../src/content-store.js";
import { listArtifacts, setArtifactArchived } from "../src/artifacts.js";
import {
  MAX_UPLOAD_BYTES,
    ...
    const app = host(db, {
      provenance: {
        decorate: async (tenantId, rows) => {
          ...
          for (const row of rows) row.sessionName = "Weekly brief";
        },
      },
    });
      ...
      provenance: {
        decorate: async (_tenantId, rows) => {
          for (const row of rows) {
            ...
            if (name !== undefined) row.sessionName = name;
          }
        },
      },
    });
...
describe("every way of not getting an artifact is indistinguishable", () => {
  ...
  test("a cross-tenant archive leaves the foreign row untouched", async () => {
    ...
    const rows = await db.execute<{ archived_at: Date | null }>(
      sql`SELECT "archived_at" FROM "artifact" WHERE "id" = ${foreign.id}`,
    );
...
describe("versions", () => {
  ...
  test("a refused skill-draft archive leaves the row untouched", async () => {
    ...
    const rows = await db.execute<{ archived_at: Date | null }>(
      sql`SELECT "archived_at" FROM "artifact" WHERE "id" = ${id}`,
    );
    expect(rows[0]!.archived_at).toBeNull();
  });
