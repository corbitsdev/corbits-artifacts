...
`scripts/dep-guard.ts` runs as `pretest` and again as a standalone CI step, failing the
build on three independently fatal categories:

1. **`@workbench/*`** — an unpublished scope. Importing it would make this package
   uninstallable for anyone outside the project that defines it.
2. **`@intx/db`** — Interchange's schema-and-client package. Taking it would bind this
   module to one host's migration story and one host's table layout; the host hands over
   a drizzle handle through `ArtifactDb` instead. (`@intx/types` is fine — types and pure
   predicates, and a declared peer.)
3. **Host workflow table names** — reading them directly would turn provenance into a
   join against a host's private schema instead of the Seam C port the host supplies.

Scope is per-directory. `src/` and `test/` carry all three; `examples/` carries only the
first, because the example *is* an Interchange host and legitimately imports `@intx/db`
to build the app the module mounts on. `node_modules`, `dist`, `build` and `coverage`
are never scanned — flagging other people's code teaches people to delete the rule
rather than fix a real leak.

Wanting to relax the guard is the signal to add a port instead.
...
- CI must be green: dep-guard, typecheck, unit + integration, build, reference-host
